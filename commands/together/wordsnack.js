        ...
        if (message.member.voice.channel) {
            client.discordTogether.createTogetherCode(message.member.voice.channel.id, 'wordsnack').then(async invite => {
                const msgEmbed = new MessageEmbed()
                    .setColor("F9A602")
                    .setTitle("Started a Words Snack play party")
                    .setURL(`${invite.code}`)
                    ...
                    .setTimestamp()
                    .setFooter('Words Snack', 'https://cdn.discordapp.com/app-icons/879863976006127627/930f9cfe504211a130419e731babc597.png?size=80&keep_aspect_ratio=false')
                return message.reply({ embeds: [msgEmbed] });
            });
        } else {
            await message.reply(`⛔ You must be in a voice channel to start a Words Snack play party`)
        }
    }
}